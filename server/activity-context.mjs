export function loadMockActivityContext(appContext, generatedAt = new Date().toISOString()) {
  const baseTime = new Date(generatedAt).getTime();
  const at = (minutesAgo) => new Date(baseTime - minutesAgo * 60 * 1000).toISOString();
  const entityContext = appContext?.entityContext && typeof appContext.entityContext === 'object'
    ? appContext.entityContext
    : {};
  const paymentId = stringOrUndefined(entityContext.paymentId) ?? 'pay_8L4K91';
  const orderId = stringOrUndefined(entityContext.orderId) ?? 'ord_20491';
  const recentErrors = Array.isArray(appContext?.recentErrors)
    ? appContext.recentErrors.map(stringOrUndefined).filter(Boolean)
    : [];
  const paymentRelated = /payment|checkout|3ds|card/i.test(
    `${appContext?.currentScreen ?? ''}_${appContext?.lastAction ?? ''}_${recentErrors.join('_')}`,
  );
  ...
  return {
    ...
    lastActions: [
      {
        ...
        detail: `Screen ${appContext?.currentScreen ?? 'unknown'} opened for order ${orderId}.`,
        outcome: 'viewed',
      },
      {
        ...
        detail: `User tapped ${appContext?.lastAction ?? 'unknown action'}; payment id ${paymentId}.`,
        outcome: 'processing',
      },
      {
        id: 'act_3ds_opened',
    ...
    deviceContext: {
      platform: stringOrUndefined(appContext?.platform),
      appVersion: stringOrUndefined(appContext?.appVersion),
      locale: stringOrUndefined(appContext?.locale),
      network: 'wifi',
